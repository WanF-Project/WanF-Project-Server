...
//    }
    ...
    public ResponseEntity<Void> checkClubAccess(@PathVariable(name = "id") Long id,
        ...
        String userAuth = clubAuthService.findByUserIdAndClubId(user.getId(), club.getId());

        if (clubService.checkClubAccess(id, clubPwdRequest, userAuth)) {
            clubAuthService.grantAuthorityToClub(user, club, "Member");
        }
    ...
    public ResponseEntity<String> getAuthority(@PathVariable(name = "id") Long id,
                                               @AuthenticationPrincipal UserDetailsImpl userDetails) {
        User user = userDetails.getUser();

        Club club = clubService.findById(id);

        String userAuth = clubAuthService.getAuthority(user.getId(), club.getId());
